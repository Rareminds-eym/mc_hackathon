...
const scenario: Scenario = {
  ...
  description: "A production worker enters the cleanroom without gloves and skips the entry logbook. Your mission: Identify the violations and deploy corrective actions!",
  pieces: [
    { id: 'v1', text: 'Personnel Hygiene', category: 'violation', isCorrect: true },
    { id: 'v2', text: 'Documentation', category: 'violation', isCorrect: true },
    { id: 'v3', text: 'Quality Control', category: 'violation', isCorrect: false },
    { id: 'v4', text: 'Equipment Qualification', category: 'violation', isCorrect: false },
    { id: 'a1', text: 'Follow gowning SOP', category: 'action', isCorrect: true },
    { id: 'a2', text: 'Sign and verify entry in log', category: 'action', isCorrect: true },
    { id: 'a3', text: 'Use cleanroom air filters', category: 'action', isCorrect: false },
    { id: 'a4', text: 'Initiate audit trail', category: 'action', isCorrect: false },
  ]
};

export const JigsawBoard = () => {
  const correctViolations = scenario.pieces.filter(p => p.category === 'violation' && p.isCorrect);
  const correctActions = scenario.pieces.filter(p => p.category === 'action' && p.isCorrect);
  ...
  useEffect(() => {
    ...
    const placedCorrectPieces = placedPieces.violations.length + placedPieces.actions.length;
  ...
  const handleDrop = (containerType: 'violations' | 'actions', piece: PuzzlePiece) => {
  ...
  const availablePieces = scenario.pieces.filter(piece => 
    !placedPieces.violations.find(p => p.id === piece.id) &&
    !placedPieces.actions.find(p => p.id === piece.id)
  );
  ...
  const dndOptions = isTouchDevice ? { enableMouseEvents: true, preview: true } : undefined;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex items-center justify-center p-4">
        <div className="text-center bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl shadow-2xl p-8 max-w-md border-2 border-cyan-400 glow-border">
          ...
          <h2 className="text-xl font-bold text-white mb-2 game-font">ROTATE DEVICE</h2>
          <p className="text-cyan-200">Switch to landscape mode to begin your mission!</p>
        </div>
      </div>
    );
  }
    ...
    <DndProvider backend={dndBackend} options={dndOptions}>
      ...
      <div className="min-h-screen h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 relative overflow-hidden flex flex-col justify-center items-center p-1">
        ...
        <div className="w-full p-1 relative z-10 flex flex-col gap-1 h-full">
          ...
          <header className="relative w-full flex flex-row items-center justify-between mb-2 px-2 py-2 bg-gradient-to-r from-gray-900/80 to-blue-900/80 rounded-2xl border-2 border-cyan-500 shadow-xl game-hud-header">
            ...
            <div className="flex flex-col items-start gap-1 min-w-[90px]">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-1 px-2 py-0.5 bg-cyan-700/80 text-cyan-100 rounded-lg font-bold text-xs border border-cyan-300 shadow game-font">AGENT 47</span>
              </div>
              <span className="mt-1 px-2 py-0.5 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-full font-bold text-xs border border-green-300 shadow game-font animate-bounce">LEVEL 3</span>
            </div>
            ...
            <div className="flex flex-col items-center flex-1">
              <h1 className="text-2xl md:text-3xl font-extrabold text-white game-font tracking-widest mb-0.5 animate-glow drop-shadow-lg flex items-center gap-2">
                ...
                <span className="bg-gradient-to-r from-yellow-300 via-orange-400 to-red-500 bg-clip-text text-transparent">MISSION: FIX THE VIOLATION</span>
                <span className="animate-bounce">⚡</span>
              </h1>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-cyan-200 text-xs md:text-base font-mono bg-black/30 px-2 py-0.5 rounded-lg border border-cyan-400 shadow-inner animate-fade-in">Deploy your GMP knowledge to complete the mission</span>
              ...
              <div className="w-full max-w-xs mx-auto h-2 bg-gray-800 rounded-full overflow-hidden border-2 border-cyan-400 mt-2 animate-fade-in">
                <div className="h-full bg-gradient-to-r from-green-400 to-cyan-400 animate-progress-bar" style={{ width: isComplete ? '100%' : `${Math.min(100, ((placedPieces.violations.length + placedPieces.actions.length) / (correctViolations.length + correctActions.length)) * 100)}%` }}></div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-end gap-2 min-w-[110px]">
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <div className="w-16 h-2 bg-gray-700 rounded-full overflow-hidden border border-red-400">
                  <div className="h-full bg-gradient-to-r from-red-500 to-green-400 animate-health-bar" style={{ width: `${health}%` }}></div>
                </div>
                <span className="text-xs font-bold text-green-300">{health}</span>
              </div>
            </div>
          </header>
          ...
          <div className="flex-1 flex flex-row gap-2 min-h-0 items-stretch overflow-x-auto">
            ...
            <div className="flex flex-row gap-2 flex-[2_2_0%] min-h-0 h-full justify-stretch items-stretch min-w-[400px] w-full">
              ...
              <div className="flex-1 flex flex-col min-h-0 items-stretch min-w-[120px] max-w-[50vw]">
                <h2 className="text-base md:text-lg font-bold text-red-400 game-font text-center mb-1">🎯 VIOLATIONS DETECTED</h2>
                <div className="flex-1 flex items-center justify-center min-h-0">
                  <div className="w-full" style={{ maxHeight: '100px', minHeight: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <JigsawContainer
                      type="violations"
              ...
              {/* Actions Container */}
              <div className="flex-1 flex flex-col min-h-0 items-stretch min-w-[120px] max-w-[50vw]">
                <h2 className="text-base md:text-lg font-bold text-blue-400 game-font text-center mb-1">⚡ DEPLOY COUNTERMEASURES</h2>
                <div className="flex-1 flex items-center justify-center min-h-0">
                  <div className="w-full" style={{ maxHeight: '100px', minHeight: '60px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <JigsawContainer
                      type="actions"
            ...
            <div className="flex-1 min-w-[80px] max-w-[80px] sm:max-w-xs flex flex-col h-full min-h-0 items-stretch justify-stretch">
              <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl shadow-2xl p-2 border-2 border-cyan-400 glow-border flex flex-col h-full min-h-0">
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <h3 className="text-xs font-bold text-white game-font">ARSENAL</h3>
                </div>
          ...
          {/* Feedback Console */}
          {feedback && (
            <div className={`mt-1 p-2 rounded-xl text-center font-bold text-xs md:text-base game-font border-2 transform transition-all duration-500 max-w-md mx-auto ${
          ...
          {/* Victory Screen */}
          {isComplete && (
            <div className="mt-1 bg-gradient-to-r from-yellow-600 via-orange-600 to-red-600 text-white p-2 rounded-xl text-center border-4 border-yellow-400 glow-border-gold relative overflow-hidden max-w-xl mx-auto">
              ...
              <div className="relative z-10">
                <div className="flex justify-center items-center gap-1 mb-1">
                  ...
                  <h2 className="text-lg font-bold game-font">MISSION COMPLETE!</h2>
                  <div className="text-lg">🏆</div>
                </div>
                {/* Removed score and combo display */}
                <p className="text-xs mb-2">Outstanding work, Agent! You've mastered GMP compliance protocols and saved the facility from contamination!</p>
                <button 
                  onClick={() => window.location.reload()}
                  className="px-4 py-1.5 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-lg font-bold hover:from-green-400 hover:to-emerald-400 transition-all duration-300 transform hover:scale-105 shadow-lg game-button text-xs"
                >
                  🚀 NEXT MISSION
                </button>
              </div>
...
const CustomDragLayer = () => {
  const { item, isDragging, sourceOffset, clientOffset } = useDragLayer((monitor) => ({
