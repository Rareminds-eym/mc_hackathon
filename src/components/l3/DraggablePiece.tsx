...
export const DraggablePiece: React.FC<DraggablePieceProps> = ({ piece }) => {
  ...
  // Futuristic neon card with glass, pixel, and cyberpunk accents
  ...
  if (piece.category === 'violation') {
    icon = <Zap className="w-4 h-4 text-pink-100 drop-shadow-glow" style={{ opacity: 0.22 }} />;
  } else if (piece.category === 'action') {
    icon = <Target className="w-4 h-4 text-cyan-100 drop-shadow-glow" style={{ opacity: 0.22 }} />;
  } else {
    icon = <Sparkles className="w-4 h-4 text-pink-100 drop-shadow-glow" style={{ opacity: 0.22 }} />;
  }
  ...
  return (
    <div
      ...
      {...attributes}
      className={`
        group relative cursor-grab active:cursor-grabbing select-none flex
        bg-gradient-to-br ${bgGradient} text-white p-4 font-extrabold text-center pixel-text tracking-wider
        shadow-[0_0_24px_4px_#f0f,0_2px_12px_0_#0008] transition-all duration-300 hover:scale-105 hover:shadow-[0_0_32px_8px_#f0f,0_2px_16px_0_#000a]
        border-4 ${borderColor} pixel-border-thick overflow-hidden
        ${isDragging ? 'opacity-70 scale-95 rotate-1 z-50 saturate-200' : 'opacity-100'}
      ...
      style={{
        ...
        filter: isDragging ? 'brightness(1.2) grayscale(0.1) blur(0.5px)' : 'brightness(1.05)',
        ...
        boxShadow: isDragging
          ? '0 0 0 4px #fff, 0 0 32px 8px #f0f, 0 0 12px 2px #fff, 0 0 0 2px #f0f8'
    ...
    >
      ...
      {/* Category Icon */}
      <div className="absolute top-2 left-2 z-30 drop-shadow-glow opacity-20">
      ...
      <div className="absolute top-2 right-2 z-30 flex gap-1">
        <Sparkles className="w-4 h-4 text-yellow-200 group-hover:animate-spin drop-shadow-glow" style={{ opacity: 0.22 }} />
        <Zap className="w-4 h-4 text-pink-300 group-hover:animate-spin drop-shadow-glow" style={{ opacity: 0.22 }} />
      </div>
      ...
      {isDragging && (
        <div 
          className="absolute inset-0 bg-pink-500/30 flex items-center justify-center z-50"
          style={{ clipPath: 'polygon(0% 15%, 8% 15%, 12% 0%, 20% 0%, 25% 15%, 75% 15%, 80% 0%, 88% 0%, 92% 15%, 100% 15%, 100% 85%, 92% 85%, 88% 100%, 80% 100%, 75% 85%, 25% 85%, 20% 100%, 12% 100%, 8% 85%, 0% 85%)', borderRadius: '12px' }}
        >
          <Sparkles className="w-7 h-7 text-yellow-300 drop-shadow-glow" style={{ opacity: 0.22 }} />
        </div>
      )}
